...
class FinancasController < ApplicationController
  ...
  def visualiza_despesas
    ...
    respond_to do |format|
      @anos = (Matinativa.first.data_matricula.year.to_i..now.year.to_i).to_a
      @meses = (1..12).to_a
      format.html          
      format.js
    end
  ...
  def visualiza_folha
    ...
    unless @mes.nil? && @ano.nil?
      ...
      @professores.each do |professor|
        valor_aula = professor.salarios.where("data_inicio <= ? AND data_fim >= ? OR data_fim IS NULL", Date.civil(@ano,@mes,1), Date.civil(@ano,@mes,-1)).first.valor
        salario = 0
        n_aulas = 0
          ...
          if horario.matriculas.any? && horario.matriculas.where("data_matricula <= ?", Date.civil(@ano,@mes,-1)).size > 0
            if horario.matriculas.size == 1 && horario.matriculas.first.data_matricula >= Date.civil(@ano,@mes,1)
              ...
              inicio.upto(fim) do |x|
                if x.wday == dia && salario_parcial < valor_aula
                  n_aulas += 1
                  n_aulas_aux += 1
                  salario_parcial += valor_aula/4
                end
              end
        ...
        Matinativa.where("professor_id = ? AND data_matricula <= ? AND termino_matricula >= ?", professor.id, Date.civil(@ano,@mes,1), Date.civil(@ano,@mes,1)).each do |inativa|
        ...
        p = {professor_id: professor.id, user_id: current_user.id, mes: @mes, ano: @ano, salario: salario, n_aulas: n_aulas}
        @folha_pagamento << p
