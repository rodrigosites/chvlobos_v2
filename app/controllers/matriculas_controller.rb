...
class MatriculasController < ApplicationController
  ...
  def index
    @matriculas_total = Matricula.order(:id)
    @pagy, @matriculas = pagy(@matriculas_total)
  end
  ...
  def new
    ...
    @alunos = Aluno.order('nome').where(ativo: true)
    @cursos = Curso.order('nome')
    unless @circular_vigente = Circular.where(vigente: true).first
    	@circular_vigente = Circular.new
      @circular_vigente.valor_extra = "Favor cadastrar uma Circular."
    	@circular_vigente.desconto = 0;
    end
  end
  ...
  def busca_dados_curso
    @curso = Curso.find(params[:curso_id])
    if @circular = Circular.where(vigente: true).first
      @valor_extra = @circular.valor_extra
    	if @curso.basico?
    		@valor_vigente = @circular.valor_mensalidade
    	else
    		@valor_vigente = @circular.valor_profissionalizante
    	end
    else
      @valor_vigente = "Não há uma Circular vigente no sistema."
    end
  end
  ...
  def encerradas
    @inativas = Matinativa.order(:id_ativa)
    @pagy, @matriculas = pagy(@inativas)
  end
  ...
  private
    ...
    def gera_contrato(matricula)
      ...
      doc.bookmarks['dia_pratica'].insert_text_after(dayPTBR(matricula.aulas.first.horario.dia).pluralize)
      doc.bookmarks['horario_pratica'].insert_text_after(hourPTBR(matricula.aulas.first.horario.horario))
      doc.bookmarks['termino_pratica'].insert_text_after(hourPTBR(matricula.aulas.first.horario.horario + 50*60))
      doc.bookmarks['professor_pratica'].insert_text_after(matricula.aulas.first.horario.professor.nome)
      if matricula.horarios.size > 1
        doc.bookmarks['dia_teoria'].insert_text_after(dayPTBR(matricula.aulas.last.horario.dia).pluralize)
        doc.bookmarks['horario_teoria'].insert_text_after(hourPTBR(matricula.aulas.last.horario.horario))
        doc.bookmarks['termino_teoria'].insert_text_after(hourPTBR(matricula.aulas.last.horario.horario + 50*60))
