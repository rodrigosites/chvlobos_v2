...
class CircularesController < ApplicationController
  ...
  def index
    @circular_vigente = Circular.where(vigente: true).first
    @pagy, @circulares_inativas = pagy(Circular.where.not(id: @circular_vigente.id).order(id: :desc))
  end
